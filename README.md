...
By gathering history systematically using standard medical frameworks (such as the SOCRATES pain framework), MediKiosk converts patient responses into an objective, SBAR-formatted AI Doctor Summary ready for instantaneous physician review.
...
- Physician Safety Guardrail: AI strictly synthesizes patient history for physician verification — it does NOT render autonomous medical diagnoses.
...
| Contributor | Branch | Component & Responsibilities |
|---|---|---|
| **Sravistha** | `frontend-language` | Welcome screen, 12 Indian Languages selector, Chief Complaint picker grid & custom text input |
| **Pushkar** | `frontend-interview` | Step-by-step Questionnaire Wizard, 6 Input Components, Navigation, Progress Bar, Structured History |
| **Siri Chandhana** | `question-tree` | `question-trees/pain.json` implementing the 8 SOCRATES pain framework categories |
| **Madhuhaas** | `backend` | Python FastAPI architecture (`backend/main.py`), PostgreSQL / SQLite engine, Database Models & API endpoints |
| **Ridhima** | `ai-summary` | `backend/services/llm_service.py` SBAR summary generator & `DoctorSummary.jsx` physician review component |
| **Rayyan** | `integration` | 8-screen complete prototype integration, CORS setup, production build validation & branch merges |
...
### 2. Run the React Frontend Kiosk UI
...
```
MediKiosk/
├── backend/                  # Python FastAPI Backend (Madhuhaas & Ridhima)
...
├── question-trees/           # SOCRATES Question Trees (Siri Chandhana)
│   └── pain.json             # SOCRATES pain evaluation framework
├── src/                      # React Frontend Source (Sravistha, Pushkar, Ridhima & Rayyan)
